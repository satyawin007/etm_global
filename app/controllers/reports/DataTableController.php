...
class DataTableController extends \Controller {
	...
	private function getIncomeTransactions($values, $length, $start){
		...
		if($search != ""){
			...
			$entities = \IncomeTransaction::where("branchId","=",$values["branch1"])->whereBetween("date",array($startdt,$enddt))->leftjoin("officebranch", "officebranch.id","=","incometransactions.branchId")->leftjoin("lookuptypevalues", "lookuptypevalues.id","=","incometransactions.lookupValueId")->select($select_args)->limit($length)->offset($start)->get();
			$total = \IncomeTransaction::where("branchId","=",$values["branch1"])->whereBetween("date",array($startdt,$enddt))->count();
			foreach ($entities as $entity){
				$entity["date"] = date("d-m-Y",strtotime($entity["date"]));
			}
		}
	...
	private function getVehicleRepairs($values, $length, $start){
		...
		$actions = array();
		$action = array("url"=>"#edit", "type"=>"modal", "css"=>"primary", "js"=>"modalEditRepairTransaction(", "jsdata"=>array("id"), "text"=>"EDIT");
		$actions[] = $action;
		$action = array("url"=>"#","css"=>"danger", "id"=>"deleteRepairTransaction", "type"=>"", "text"=>"DELETE");
		$actions[] = $action;
		$action = array("url"=>"#edit", "type"=>"modal", "css"=>"info", "js"=>"modalViewRepairTransactionItems(", "jsdata"=>array("id"), "text"=>"VIEW ITEMS");
		$actions[] = $action;
		$values["actions"] = $actions;
