@extends('masters.master')
	...
	@section('page_content')
		...
		<div class="row ">
		<div class="col-xs-offset-0 col-xs-12" style="max-width:99%">
	...
	@section('inline_js')
		...
		<script type="text/javascript">
			...
			$("#genreportbtn").on("click",function(){
				...
				if(provider == "stockpurchase"){
					myTable.ajax.url("getreportsdatatabledata?name=stockpurchase&warehouse="+warehouse+"&fromdate="+fromdate+"&todate="+todate).load();
				}
				if(provider == "vehiclestockhistory"){
					myTable.ajax.url("getreportsdatatabledata?name=vehiclestockhistory&vehicle="+vehicle+"&fromdate="+fromdate+"&todate="+todate).load();
				}
				if(provider == "repairstock"){
					myTable.ajax.url("getreportsdatatabledata?name=repairstock&creditsupplier="+creditsupplier+"&item="+item+"&fromdate="+fromdate+"&todate="+todate).load();
				}
			})
			...
			jQuery(function($) {		
				...
				.DataTable( {
			        ...
			        "bServerSide": true,
					"ajax":{
		                url :"getreportsdatatabledata?name=<?php echo $values["provider"] ?>", // json datasource
