...
?>
	...
	@stop	
		...
		<div class="widget-box col-xs-12">
			...
			<div class="widget-body"  ng-app="myApp" id="myCtrl" ng-controller="myCtrl">
				...
				<form style="padding-top:20px;" class="{{$form_info['class']}}" action="{{$form_info['action']}}" method="{{$form_info['method']}}" name="{{$form_info['name']}}"  id="{{$form_info['name']}}">
					...
					<div >
						...
						<?php foreach ($form_fields as $form_field) {?>
							<div class="col-xs-12">
							<?php if($form_field['type'] === "text" || $form_field['type'] === "email" ||$form_field['type'] === "number" || $form_field['type'] === "password"){ ?>
							<div id="div_{{$form_field['name']}}" class="form-group" >
								<label class="col-xs-4 control-label no-padding-right" for="form-field-1"> <?php echo strtoupper($form_field['content']); if($form_field['required']=="required") echo '<span style="color:red;">*</span>'; ?> </label>
								<div class="col-xs-8">
							...
							<?php if($form_field['type'] === "checkbox"){ ?>
								<div id="div_{{$form_field['name']}}" class="form-group">
									...
									<div class="col-xs-8">
										...
										<div class="checkbox inline">
											<label>
												<input name="{{$key}}" id="{{$key}}" ng-model="{{$key}}" value="{{$value}}" type="checkbox" class="ace">
												<span class="lbl">&nbsp;{{$value}} &nbsp;&nbsp;</span>
											</label>
										</div>
										<?php } ?>
						...
						<div class="col-xs-8">
							<table class="table table-striped table-bordered table-hover" style="border: 1px solid #DDDDDD;" >
								...
								<tr><td colspan="7" style="text-align: center; font-weight: bold">PREVIOUS DATES LOGS (Upto 5 Days)<td></tr>
								<tr ng-repeat="exe_rec in exe_recs_text">
									<td>@{{exe_rec.vehicle}}</td>
									<td>@{{exe_rec.servicedate}}</td>
									<td>@{{exe_rec.reading}}</td>
									<td>@{{exe_rec.drivers}}
									<td>@{{exe_rec.helper}}</td>
									<td>@{{exe_rec.remarks}}</td>
									<td>@{{exe_rec.status}}</td>
									<td>
										NO ACTION
									</td>
								</tr>
