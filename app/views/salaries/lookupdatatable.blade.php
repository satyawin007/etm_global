		...
		<script type="text/javascript">
			...
			function deleteSalryAdvance(id) {
				bootbox.confirm("Are you sure, you want to delete this booking?", function(result) {
					if(result) {
						$.ajax({
					      url: "deletesalaryadvance?id="+id,
					      success: function(data) {
						      if(data=="success"){
						    	  bootbox.confirm('SALARY ADVANCE SUCCESSFULLY DELETED!', function(result) {});
						      }
						      else{
						    	  bootbox.confirm('SALARY ADVANCE COULD NOT BE DELETED!', function(result) {});
						      }
					      },
					      type: 'GET'
					   });	
			...
			jQuery(function($) {		
				...
				.DataTable( {
			        ...
			        "bServerSide": true,
					"ajax":{
		                url :"getsalarydatatabledata?name=<?php echo $values["provider"] ?>", // json datasource
